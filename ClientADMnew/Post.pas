...
uses
  ...
  StdCtrls, DBCtrls, Db, DBClient, Grids, DBGrids, ExtCtrls, Buttons,
...
begin
  ...
  Timer1.Enabled:=true;
  tek_id:=0;
  ch_mail:=fr_ClientAdm.MIDASConnection1.AppServer.CheckNewMessages;
  lb_new_msg.Caption:='Новых сообщений: '+IntToStr(ch_mail);
  Timer1.Enabled:=true;
end;
...
begin
  If cds_in_messages.FieldByName('id').AsInteger>0 Then Begin
    ...
    StartReadMsq(DateTimeToStr(cds_in_messages.FieldByName('send_date').AsDateTime),
                'Автор письма: '+cds_in_messages.FieldByName('sender').AsString,
...
procedure Tfr_post_list.FormShow(Sender: TObject);
begin
   If cds_in_messages.Active Then Begin
       cds_in_messages.Last;
   end;
end;
